...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  const provider = path.includes("github") ? "github" : "discord";
  ...
  try {
    ...
    if (provider === "github") {
      ...
      const userRes = await fetch("https://api.github.com/user", {
        headers: { Authorization: `Bearer ${tokenData.access_token}`, "User-Agent": "Involvex-Portal" },
      });
    ...
    if (!user) {
      const username = provider === "github" ? (userProfile as GitHubUserProfile).login : (userProfile as DiscordUserProfile).username;
      const email = userProfile.email || null;
      user = await createUser(db, email, null, username, { provider, id: oauthId });
    }
    ...
    const errorMessage = error instanceof Error ? error.message : "A non-Error was thrown";
