...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  try {
    ...
    const [githubRes, npmRes, userSubs] = await Promise.all([
      ...
      db.prepare("SELECT * FROM subscriptions WHERE user_id = ? AND is_active = 1").bind(userId).all<Subscription>()
    ]);
...
export default function Dashboard() {
  const { user, github, npm, subscriptions, stats } = useLoaderData<typeof loader>();

  return (
    <div className="container">
      <div style={{ marginBottom: '2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h1 className="hacker-text" style={{ margin: 0 }}>Terminal: {user.username || user.email}</h1>
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
          {user.role === 'admin' && (
            <Link to="/admin" style={{ color: 'var(--color-secondary)', fontSize: '0.9rem' }}>[ ADMIN_PANEL ]</Link>
          )}
          <Link to="/profile" style={{ color: 'var(--color-accent-green)', fontSize: '0.9rem' }}>[ PROFILE ]</Link>
          <Form action="/auth/logout" method="post">
            <button type="submit" style={{ background: 'none', border: 'none', color: 'var(--color-error-red)', cursor: 'pointer', fontFamily: 'inherit', fontSize: '0.9rem' }}>
              [ DISCONNECT ]
            </button>
          </Form>
        </div>
      ...
      <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', marginBottom: '2rem' }}>
        ...
        <StatsCard title="Your Subs" value={stats.totalSubscriptions} icon="star" />
      </div>
      ...
      <footer style={{ marginTop: '4rem', padding: '2rem 0', borderTop: '1px solid var(--color-dark-green)', textAlign: 'center', color: 'var(--color-text-grey)', fontSize: '0.8rem' }}>
        Involvex Neural Link v0.0.3 • Connection Secure • ID: {user.id.slice(0, 8)}
      </footer>
    </div>
  );
}